...
async def create_link(request: Request, _=Depends(require_auth)):
    ...
    async with LINKS_LOCK:
        if any(data["label"] == label for data in LINKS.values()):
    ...
    if max_conn < 0:
        max_conn = 0
    uid = generate_uuid()  # Random UUID for each user
    async with LINKS_LOCK:
...
html[data-theme="dark"]{--bg:#000000;--surface:rgba(20,20,20,0.95);--surface2:#1a1a1a;--border:rgba(255,255,255,0.08);--text:rgba(255,255,255,0.95);--text2:rgba(255,255,255,0.5);--text3:rgba(255,255,255,0.2);--primary:#ffffff;--primary-glow:rgba(255,255,255,0.05);--accent:#cccccc;--error:#ff4444;--error-bg:rgba(255,68,68,0.08);--orb1:rgba(255,255,255,0.03);--orb2:rgba(255,255,255,0.02);--orb3:rgba(255,255,255,0.01)}
html[data-theme="light"]{--bg:#ffffff;--surface:#ffffff;--surface2:#f5f5f5;--border:rgba(0,0,0,0.08);--text:#000000;--text2:rgba(0,0,0,0.5);--text3:rgba(0,0,0,0.2);--primary:#000000;--primary-glow:rgba(0,0,0,0.05);--accent:#333333;--error:#ff0000;--error-bg:rgba(255,0,0,0.05);--orb1:rgba(0,0,0,0.02);--orb2:rgba(0,0,0,0.01);--orb3:rgba(0,0,0,0.005)}
...
.orb{position:absolute;border-radius:50%;filter:blur(100px);opacity:0;animation:orbFloat 20s ease-in-out infinite}
.orb-1{width:500px;height:500px;background:var(--orb1);top:-15%;left:-10%;animation-delay:0s}
.orb-2{width:400px;height:400px;background:var(--orb2);bottom:-15%;right:-10%;animation-delay:-7s}
.orb-3{width:300px;height:300px;background:var(--orb3);top:30%;left:50%;animation-delay:-14s}
@keyframes orbFloat{0%,100%{transform:translate(0,0) scale(1);opacity:0.4}25%{transform:translate(80px,-50px) scale(1.1);opacity:0.6}50%{transform:translate(-40px,60px) scale(0.9);opacity:0.3}75%{transform:translate(50px,30px) scale(1.05);opacity:0.5}}
.toolbar{position:fixed;top:20px;right:20px;display:flex;gap:8px;z-index:10}
.toolbar button{width:38px;height:38px;border-radius:10px;border:1px solid var(--border);background:var(--surface);color:var(--text2);cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:13px;transition:all .3s;backdrop-filter:blur(20px)}
...
.login-card{background:var(--surface);border:1px solid var(--border);border-radius:28px;padding:50px 40px 40px;position:relative;overflow:hidden;backdrop-filter:blur(40px);box-shadow:0 8px 40px rgba(0,0,0,0.2);animation:cardIn .8s cubic-bezier(0.16,1,0.3,1) forwards;opacity:0;transform:translateY(30px) scale(0.96)}
...
.brand svg{margin-bottom:20px;filter:drop-shadow(0 0 30px rgba(255,255,255,0.05));animation:logoPulse 4s ease-in-out infinite}
@keyframes logoPulse{0%,100%{filter:drop-shadow(0 0 20px rgba(255,255,255,0.05));transform:scale(1)}50%{filter:drop-shadow(0 0 40px rgba(255,255,255,0.1));transform:scale(1.02)}}
.brand h1{font-size:26px;font-weight:900;color:var(--text);letter-spacing:-0.03em;animation:fadeUp .6s .2s ease both}
.brand p{font-size:12px;color:var(--text3);margin-top:8px;font-weight:600;letter-spacing:0.15em;text-transform:uppercase;animation:fadeUp .6s .3s ease both}
@keyframes fadeUp{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}
.form-group{margin-bottom:22px;animation:fadeUp .6s .4s ease both}
.form-group label{display:block;font-size:12px;font-weight:700;color:var(--text2);margin-bottom:8px;text-transform:uppercase;letter-spacing:0.08em}
...
.login-btn{width:100%;padding:14px;background:var(--primary);border:none;border-radius:14px;color:var(--bg);font-size:14px;font-weight:800;font-family:inherit;cursor:pointer;transition:all .3s cubic-bezier(0.4,0,0.2,1);letter-spacing:0.02em;position:relative;overflow:hidden;animation:fadeUp .6s .5s ease both}
.login-btn:hover{transform:translateY(-2px);box-shadow:0 8px 30px var(--primary-glow)}
.login-btn:active{transform:translateY(0) scale(0.98)}
.error-msg{background:var(--error-bg);border:1px solid rgba(255,68,68,0.15);color:var(--error);padding:10px 16px;border-radius:12px;font-size:13px;display:none;margin-bottom:20px;text-align:center;font-weight:500;animation:shake .4s ease}
  ...
  <div class="login-card" id="login-card">
    <div class="brand">
      <svg width="60" height="60" viewBox="0 0 56 56" fill="none">
        ...
        <circle cx="28" cy="28" r="14" stroke="var(--bg)" stroke-width="1.5" opacity="0.3">
          <animateTransform attributeName="transform" type="rotate" from="0 28 28" to="360 28 28" dur="20s" repeatCount="indefinite"/>
        </circle>
        <circle cx="28" cy="18" r="3.5" fill="var(--bg)">
          <animate attributeName="r" values="3.5;4;3.5" dur="3s" repeatCount="indefinite"/>
        </circle>
        <circle cx="19" cy="33" r="3.5" fill="var(--bg)">
          <animate attributeName="r" values="3.5;4;3.5" dur="3s" begin="1s" repeatCount="indefinite"/>
        </circle>
        <circle cx="37" cy="33" r="3.5" fill="var(--bg)">
          <animate attributeName="r" values="3.5;4;3.5" dur="3s" begin="2s" repeatCount="indefinite"/>
        </circle>
        <line x1="28" y1="21.5" x2="21" y2="30" stroke="var(--bg)" stroke-width="1.5" opacity="0.8"/>
        <line x1="28" y1="21.5" x2="35" y2="30" stroke="var(--bg)" stroke-width="1.5" opacity="0.8"/>
        <line x1="22.5" y1="33" x2="33.5" y2="33" stroke="var(--bg)" stroke-width="1.5" opacity="0.8"/>
        <circle cx="28" cy="28" r="2" fill="var(--bg)" opacity="0.9"/>
      </svg>
      <h1>WireXnet</h1>
      <p>v1.0</p>
    </div>
...
<script>
let theme=localStorage.getItem('wirex_theme')||'dark';
function applyTheme(t){theme=t;document.documentElement.setAttribute('data-theme',t);localStorage.setItem('wirex_theme',t)}
...
<style>
*{margin:0;padding:0;box-sizing:border-box}
html[data-theme="dark"]{--bg:#000000;--surface:#0a0a0a;--surface2:#141414;--surface3:#1c1c1c;--border:rgba(255,255,255,0.06);--border2:rgba(255,255,255,0.1);--text:#ffffff;--text2:rgba(255,255,255,0.5);--text3:rgba(255,255,255,0.2);--primary:#ffffff;--primary-glow:rgba(255,255,255,0.05);--primary-dim:rgba(255,255,255,0.05);--green:#00ff88;--green-dim:rgba(0,255,136,0.05);--red:#ff4444;--red-dim:rgba(255,68,68,0.05);--yellow:#ffaa00;--sidebar-bg:#050505;--shadow:0 1px 3px rgba(0,0,0,0.4)}
html[data-theme="light"]{--bg:#ffffff;--surface:#fafafa;--surface2:#f0f0f0;--surface3:#e8e8e8;--border:rgba(0,0,0,0.06);--border2:rgba(0,0,0,0.1);--text:#000000;--text2:rgba(0,0,0,0.5);--text3:rgba(0,0,0,0.2);--primary:#000000;--primary-glow:rgba(0,0,0,0.05);--primary-dim:rgba(0,0,0,0.05);--green:#00aa55;--green-dim:rgba(0,170,85,0.05);--red:#cc0000;--red-dim:rgba(204,0,0,0.05);--yellow:#cc8800;--sidebar-bg:#fafafa;--shadow:0 1px 3px rgba(0,0,0,0.06)}
html,body{height:100%}
body{font-family:'Inter',-apple-system,BlinkMacSystemFont,sans-serif;background:var(--bg);color:var(--text);min-height:100vh;display:flex;transition:background .3s,color .3s}
...
.sidebar-brand{padding:20px 16px 16px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--border);position:relative;overflow:hidden}
...
@keyframes shimmer{0%,100%{opacity:0.2;transform:scaleX(0.2)}50%{opacity:0.6;transform:scaleX(1)}}
...
.sidebar-brand-right button{width:30px;height:30px;border-radius:8px;border:1px solid var(--border);background:var(--surface);color:var(--text3);cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:12px;transition:all .2s}
.sidebar-brand-right button:hover{border-color:var(--primary);color:var(--primary)}
.sidebar-nav{flex:1;padding:12px 8px;overflow-y:auto}
.nav-section{font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:0.12em;padding:16px 12px 8px}
.nav-item{display:flex;align-items:center;gap:12px;padding:10px 14px;margin:2px 0;border-radius:10px;color:var(--text2);font-size:13px;font-weight:500;cursor:pointer;transition:all .15s;text-decoration:none;border:none;background:none;width:100%;text-align:left}
.nav-item:hover{background:var(--primary-dim);color:var(--text)}
.nav-item.active{background:var(--primary-dim);color:var(--text);font-weight:600;box-shadow:inset 3px 0 0 var(--primary)}
.nav-icon{width:18px;height:18px;flex-shrink:0;opacity:0.6}
.nav-item.active .nav-icon{opacity:1}
.nav-badge{margin-left:auto;background:var(--surface3);color:var(--text3);font-size:10px;padding:2px 8px;border-radius:8px;font-weight:600}
.sidebar-footer{padding:12px 16px;border-top:1px solid var(--border)}
.sidebar-footer .logout-btn{width:100%;padding:8px;border:1px solid var(--border);border-radius:8px;background:none;color:var(--text3);font-family:inherit;font-size:11px;font-weight:600;cursor:pointer;transition:all .2s;display:flex;align-items:center;justify-content:center;gap:8px}
.sidebar-footer .logout-btn:hover{background:var(--red-dim);border-color:var(--red-dim);color:var(--red)}
.sidebar-footer .version{text-align:center;font-size:10px;color:var(--text3);margin-top:10px;letter-spacing:0.04em}

.main{margin-left:220px;flex:1;padding:28px 32px 48px;min-height:100vh}
...
.page-sub{font-size:12px;color:var(--text3);margin-top:4px;font-weight:500}

.stats-row{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:20px}
.stat-card{background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:18px 20px;transition:all .3s cubic-bezier(0.4,0,0.2,1);animation:cardIn .5s ease both}
...
.stat-value{font-size:24px;font-weight:800;color:var(--text);letter-spacing:-0.02em}
.stat-unit{font-size:13px;font-weight:400;color:var(--text3)}

.card{background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:20px;margin-bottom:14px;transition:all .3s cubic-bezier(0.4,0,0.2,1);animation:cardIn .5s ease both}
...
.btn{font-family:inherit;font-size:12px;font-weight:600;border-radius:10px;padding:8px 16px;cursor:pointer;display:inline-flex;align-items:center;gap:6px;border:none;transition:all .15s}
...
.table th{text-align:left;font-size:11px;font-weight:600;color:var(--text3);padding:12px 14px;text-transform:uppercase;letter-spacing:0.06em;border-bottom:1px solid var(--border);background:var(--surface2)}
...
.tag{display:inline-flex;align-items:center;padding:3px 10px;border-radius:6px;font-size:10px;font-weight:700;letter-spacing:0.04em;text-transform:uppercase}
...
.tag-offline{background:var(--text3);color:var(--text2)}

.usage-pill{display:flex;align-items:center;gap:10px;padding:4px 12px;border-radius:999px;background:var(--surface3);font-size:11px;color:var(--text2)}
.usage-pill .used{color:var(--text);font-weight:600}
.usage-pill .bar{flex:1;height:4px;background:var(--bg);border-radius:2px;min-width:60px}
.usage-pill .fill{height:100%;border-radius:2px;transition:width .3s}
.usage-pill .limit{color:var(--text3)}

.toggle{width:36px;height:20px;border-radius:10px;background:var(--surface3);position:relative;cursor:pointer;transition:all .3s cubic-bezier(0.4,0,0.2,1);border:1px solid var(--border)}
.toggle::after{content:'';position:absolute;width:14px;height:14px;border-radius:50%;background:var(--text3);top:2px;left:2px;transition:all .3s cubic-bezier(0.4,0,0.2,1)}
...
.form-input,.form-select{padding:10px 14px;border-radius:10px;border:1px solid var(--border);font-family:inherit;font-size:13px;outline:none;color:var(--text);background:var(--surface2);transition:all .2s}
...
.toast{position:fixed;bottom:24px;left:50%;transform:translateX(-50%) translateY(20px);background:var(--surface);color:var(--text);border:1px solid var(--border);border-radius:12px;padding:12px 24px;font-size:12px;font-weight:500;opacity:0;transition:all .3s cubic-bezier(0.4,0,0.2,1);z-index:999;display:flex;align-items:center;gap:8px;box-shadow:0 8px 24px rgba(0,0,0,0.3);backdrop-filter:blur(20px)}
.toast.show{opacity:1;transform:translateX(-50%) translateY(0)}
.toast.error{border-color:var(--red-dim);color:var(--red)}

.modal-overlay{position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:200;display:none;align-items:center;justify-content:center;backdrop-filter:blur(8px)}
.modal-overlay.show{display:flex}
.modal{background:var(--surface);border:1px solid var(--border);border-radius:18px;padding:28px 32px;width:100%;max-width:480px;position:relative;box-shadow:0 20px 60px rgba(0,0,0,0.4);transform:scale(0.9);opacity:0;transition:all .4s cubic-bezier(0.34,1.56,0.64,1)}
...
.modal-close{position:absolute;top:14px;right:16px;background:var(--surface3);border:1px solid var(--border);color:var(--text3);width:30px;height:30px;border-radius:8px;cursor:pointer;font-size:14px;display:flex;align-items:center;justify-content:center;transition:all .2s}
...
.qr-box img{max-width:220px;border-radius:10px;border:3px solid var(--surface);box-shadow:0 4px 16px rgba(0,0,0,0.1)}
.btn-copy,.btn-qr{font-family:inherit;font-size:11px;font-weight:600;border-radius:8px;padding:5px 12px;cursor:pointer;border:none;display:inline-flex;align-items:center;gap:4px;transition:all .2s}
.btn-copy{background:var(--primary-dim);color:var(--text);border:1px solid var(--border)}
.btn-copy:hover{background:var(--primary);color:var(--bg)}
.btn-qr{background:var(--green-dim);color:var(--green);border:1px solid var(--green-dim)}
.btn-qr:hover{background:var(--green);color:var(--bg)}

.detail-label{font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:0.06em;margin-bottom:5px}
...
.inbounds-toolbar{display:flex;align-items:center;gap:10px;margin-bottom:14px;flex-wrap:wrap}
.search-box{flex:1;min-width:200px;position:relative}
.search-box input{width:100%;padding:10px 14px 10px 36px;background:var(--surface2);border:1px solid var(--border);border-radius:10px;color:var(--text);font-size:12px;font-family:inherit;outline:none;transition:all .2s}
...
.filter-chips{display:flex;gap:4px;padding:4px 6px;background:var(--surface2);border:1px solid var(--border);border-radius:10px}
.chip{padding:6px 14px;border-radius:6px;font-size:11px;font-weight:600;color:var(--text3);cursor:pointer;border:none;background:none;transition:all .2s;font-family:inherit}
.chip.active{background:var(--primary);color:var(--bg)}
.chip:hover:not(.active){background:var(--surface3);color:var(--text2)}

.inbound-cards{display:none;flex-direction:column;gap:10px;padding:0 4px}
.inbound-card{border:1px solid var(--border);border-radius:12px;padding:14px;background:var(--surface2);display:flex;flex-direction:column;gap:10px}
...
.inbound-card-name{font-size:14px;font-weight:600;color:var(--text)}
.inbound-card-actions{display:flex;gap:4px;justify-content:flex-end}

.mobile-header{display:none;position:fixed;top:0;left:0;right:0;height:50px;background:var(--sidebar-bg);border-bottom:1px solid var(--border);z-index:90;align-items:center;justify-content:space-between;padding:0 16px}
.menu-toggle{width:36px;height:36px;border-radius:10px;border:1px solid var(--border);background:var(--surface);color:var(--text2);display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:16px}
...
@media(max-width:768px){
  ...
  .main{margin-left:0;padding-top:66px;padding-left:14px;padding-right:14px}
...
<div class="mobile-header">
  <span style="font-weight:800;font-size:14px">WireXnet</span>
...
<main class="main">
  ...
  <section class="page active" id="page-dashboard">
    ...
    <div class="stats-row">
      ...
      <div class="stat-card">
        ...
        <div class="stat-value" id="s-domain" style="font-size:12px;word-break:break-all;font-weight:600">--</div>
      </div>
    </div>
    <div class="grid-2">
      <div class="card">
  ...
  <section class="page" id="page-inbounds">
    ...
    <div class="card" style="border-radius:14px;overflow:hidden;padding:0">
  ...
  <section class="page" id="page-addresses">
    <div class="page-header">
      <div>
        ...
        <div class="page-sub">IPs and domains for subscription configs</div>
      </div>
      <button class="btn btn-primary" onclick="showAddAddressModal()">+ Add</button>
    </div>
    <div class="card">
      <div class="card-header"><div class="card-title">Clean IP List</div></div>
      <div class="status-item" style="flex-direction:column;gap:10px">
...
<div class="modal-overlay" id="add-address-modal" onclick="if(event.target===this)this.classList.remove('show')">
  <div class="modal" style="position:relative">
    ...
    <button class="btn btn-primary" onclick="addAddresses()" style="width:100%;margin-top:8px;justify-content:center">Add All</button>
  </div>
</div>
...
async function loadStats(){
  try{
    ...
    const pulse=(el,val)=>{if(el&&el.textContent!==val){el.style.transition='color .2s';el.style.color='var(--primary)';el.textContent=val;setTimeout(()=>el.style.color='',400)}};
    if($('#s-traffic')){$('#s-traffic').innerHTML=statsData.total_traffic_mb+'<span class="stat-unit"> MB</span>'}
...
function renderLinks(links){
  ...
  tbody.innerHTML=rows.map(r=>`<tr>
    ...
    <td><span class="tag ${r.l.active?'tag-active':'tag-disabled'}">${r.l.active?'On':'Off'}</span> <span class="tag ${r.isOnline?'tag-online':'tag-offline'}">${r.isOnline?'Online':'Offline'}</span></td>
  ...
  cards.innerHTML=rows.map(r=>`<div class="inbound-card">
    ...
    <div style="margin-top:6px"><span class="tag ${r.l.active?'tag-active':'tag-disabled'}">${r.l.active?'On':'Off'}</span> <span class="tag ${r.isOnline?'tag-online':'tag-offline'}">${r.isOnline?'Online':'Offline'}</span></div>
...
function copyLinkText(txt){navigator.clipboard.writeText(txt).then(()=>toast('Copied')).catch(()=>toast('Failed',true))}
...
function downloadQR(){const img=$('#qr-img');if(!img.src)return;const a=document.createElement('a');a.href=img.src;a.download='wirex-qr.png';a.click()}
async function copySubLink(uid){try{const domain=location.host;const subUrl=`https://${domain}/sub/${uid}`;await navigator.clipboard.writeText(subUrl);toast('Subscription URL copied');}catch(e){toast('Failed',true)}}
...
setInterval(()=>{loadStats();loadLinks();},8000);
...
function renderAddresses(){
  ...
  list.innerHTML=allAddresses.map((a,i)=>`
    <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:var(--surface2);border:1px solid var(--border);border-radius:10px">
      ...
      <button class="btn btn-danger btn-sm" onclick="deleteAddress(${i})" style="padding:4px 10px">✕</button>
    </div>
  `).join('');
}
...
function initChart(){
  ...
  trafficChart=new Chart(ctx,{
    ...
    options:{
      ...
      scales:{x:{grid:{display:false},ticks:{color:textColor,font:{size:10}}},y:{grid:{color:gridColor},ticks:{color:textColor,font:{size:10},callback:v=>v+' MB'},beginAtZero:true}}
    }
  });
}
initChart();
